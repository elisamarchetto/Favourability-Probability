...
Random.SDM <- function(alpha=-0.05,value=NULL,Predictors=NULL, SamplePoints=NULL) {
  ...
  Model<-multGLM(modSpecies, sp.cols = 1, var.cols=2:ncol(modSpecies), family = "binomial",
                 step = FALSE, FDR = FALSE, trim = FALSE, Y.prediction = FALSE, 
                 P.prediction = TRUE, Favourability = TRUE) 
  
  Pred<- getPreds(preds, models=Model$models, id.col = NULL, Y = FALSE, P = TRUE,
                  Favourability = TRUE)
...
Stratified.SDM <- function(Predictors=NULL, alpha= -0.05) {
  ...
  for (i in 1:length(myRandNum)) {
    ...
    b <- as(extent(5,15,45,50), 'SpatialPolygons')
    p <- erase(p, b) 
    ...
    for(j in 1:5) {
      if(j==1){
        ...
        Model<-multGLM( modSpecies[[j]], sp.cols = 1, var.cols=2:ncol( modSpecies[[j]]), family = "binomial",
                        step = FALSE, FDR = FALSE, trim = FALSE, Y.prediction = FALSE, 
                        P.prediction = TRUE, Favourability = TRUE) 
...
########### RANDOM FOREST #########
  ...
  Model<-ranger(modSpecies$pres ~., data= modSpecies, importance='impurity') 
  Pred<- predict(
    Model,
    data =preds,
    predict.all = FALSE,
...
Stratified.SDM <- function(Predictors=NULL, alpha= -0.05) {
  ...
  Model<-ranger(modSpecies[[j]]$pres ~., data= modSpecies[[j]], importance='impurity') 
  Pred<- predict(
    Model,
    data =preds,
    predict.all = FALSE,
