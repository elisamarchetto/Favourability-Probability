...
df.Tot <- rbind(df.diff_probfavGLM.R, df.diff_probfavGLM.S, df.diff_probfavRF.R, df.diff_probfavRF.S, df.diff_probfavGAM.R, df.diff_probfavGAM.S, df.diff_probfavGBM.R, df.diff_probfavGBM.S)
  ...
  labs(x="Model", y="CV difference value", fill="Sampling method")+
...
########### Maps CV favourability probability and difference for each statistical model ###########
     ...
     s1<-gplot(stack_probS) + 
       ...
       theme(plot.margin = margin(.3,.3,.3,.3, "cm"),
             ...
             axis.ticks.y=element_blank())+
       guides(fill = guide_colourbar(title.position="top", title.hjust = 0.5, barwidth = 10, barheight = 0.6),
              size = guide_legend(title.position="top", title.hjust = 0.5))+
       coord_equal(xlim = c(-26, 41), ylim = c(30, 72), expand = TRUE)
     ...
     s3<-gplot(stack_diffS) + 
       geom_tile(aes(fill = value)) +
       facet_wrap(~ variable, nrow = 4) + 
       scale_fill_scico(palette = "batlow",direction = 1,alpha = 0.7, limits=c(-64.161011,49.23872),
                        oob = scales::squish, na.value="transparent")+
       labs(x="Longitude",y="Latitude", fill="Difference values", title="Difference")+
       theme_light()+
       theme(plot.margin = margin(.3,.3,.3,.3, "cm"),
         legend.position = "bottom",  
             plot.title = element_text(size=15,face = 'bold',hjust = 0.5),
    ...
    s <- s1 + s2 + s3 + plot_annotation(title = "Stratified sampling")
    
    
    
    
    
    r1<-gplot(stack_probR) + 
      geom_tile(aes(fill = value)) +
      facet_wrap(~ variable, nrow = 4) + 
      scale_fill_scico(palette = "batlow",direction = 1,alpha = 0.7, limits=c(0.00000000,223.6068),
                       oob = scales::squish, na.value="transparent")+
    ...
    r3<-gplot(stack_diffR) + 
      geom_tile(aes(fill = value)) +
      facet_wrap(~ variable, nrow = 4) + 
      scale_fill_scico(palette = "batlow",direction = 1,alpha = 0.7, limits=c(-66.447537, 50.29264),
                       oob = scales::squish, na.value="transparent")+
      labs(x="Longitude",y="Latitude", fill="Difference values", title="Difference")+
      theme_light()+
      theme(plot.margin = margin(.3,.3,.3,.3, "cm"),
    ...
    r <- r1 + r2 + r3 + plot_annotation(title = "Random sampling")
